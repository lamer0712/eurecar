...
% obs_temp = [obs ; obs_1];
obs_temp = obs;
lane_temp = deresolution(lane,gap*0.75,1,1);
% lane_temp = lane;
n0 = size(lane_temp,1);
...
for kk = 1;
    ...
    rt = 2*rangete*rand(n0,1)-rangete + pos(3) - pi/2; % initial angle + pi/2
    ...
    obs_temp = [obs_temp; rx-offset(1) ry-offset(2)];
%         obs_1 = [rx-offset(1) ry-offset(2)];
%         save('obs_1.mat','obs_1');
    
    if(sqrt(sqrt(weight_best(kk)))>70)
...
if(lane_out ~= 0)
    ...
    parkt = atan2((model_sub(27,2) - model_sub(16,2)) , (model_sub(27,1) - model_sub(16,1)))-0.01;
   
    park_point = parking_point([model_sub(32,1) model_sub(32,2) model_sub(27,1) model_sub(27,2)], pos);
    park = [parkx-2*cos(parkt) parky-2*sin(parkt) parkt];
    park_1 = [parkx+6*cos(parkt) parky+6*sin(parkt) parkt];
    
    waypoint = parking_path(park_point, pos);
    waypoint_1 = parking_path(park_1, park);
    waypoint_2 = parking_path(park_point, park_1);
    
    path_for = pt2path(waypoint,0.15,2.77778,-1.73611,200,0.186805,10);
%     path_for = [1 1];
    path_back = pt2path([waypoint_1;waypoint_2],0.15,2.77778,-1.73611,200,0.186805,10);
end